...
int main (int argc, char **argv)
{
    ...
    {
	cout << all_degree[i].
	    vertex << " " << all_degree[i].out_degree << " " << all_degree[i].
	    in_degree << endl;
    }
    ...
    //pair the more-indegree node with more outdegree node
    cout << "MCF" << endl;
    Path *path;
    path = new Path ();
	...
	while (out_list[i].diff_degree > 0)
	{
		...
		{
		    std::vector < std::vector < Edge * >>find_paths =
			path->find_paths (out_list[i].name, in_list[j].name);
	    ...
	    std::vector < std::vector < Edge * >>avail_paths =
		path->find_paths (min_name, out_list[i].name);
	    path->debug ();
    ...
    // using gplot to export a  plot file(the flow indicates how many times the edge need to be used)
    Gplot *gp = new Gplot ();
    gp->gp_add (nm->elist);
    gp->gp_dump (true);
    gp->gp_export ("middle");

    //find walk(create array for all edge of walk)
    class SIMPLE_EDGE
    {
      public:
	string head;
	...
	cout << tra->head->name << "->" << tra->tail->
	    name << ":" << tra->flowval << endl;
	tra = tra->next;
    }
    cout << endl;
    ...
    myfile << "answer path:" << endl;
    cout << "answer path:" << endl;
    for (int i = 0; i < all_edge.size (); i++)
    {
	cout << all_edge[i].head << "->" << all_edge[i].tail << endl;
	myfile << all_edge[i].head << "->" << all_edge[i].tail << endl;
    }
    myfile.close ();
